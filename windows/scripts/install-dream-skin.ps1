...
try {
  ...
  if (-not $NoShortcuts) {
    ...
    $startScript = Join-Path $PSScriptRoot 'launch-windows.ps1'
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      ...
      $shortcut.Arguments = "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$startScript`"$portArgument"
    ...
    $restore.Arguments = "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$restoreScript`"$portArgument"
