...
try {
  ...
  [System.IO.File]::WriteAllBytes((Join-Path $fakePackageRoot 'app\chrome.dll'), [byte[]]@())
  New-Item -ItemType Directory -Path (Join-Path $fakePackageRoot 'app\resources') -Force | Out-Null
  [System.IO.File]::WriteAllBytes((Join-Path $fakePackageRoot 'app\resources\app.asar'), [byte[]]@())
  $fakeManifest = @'
...
'@
  Write-DreamSkinUtf8FileAtomically -Path (Join-Path $fakePackageRoot 'AppxManifest.xml') `
    -Content $fakeManifest
  ...
  if ($null -eq $fakeInstall -or $fakeInstall.PackageFullName -cne $fakePackage.PackageFullName -or
    -not (Test-DreamSkinPathEqual -Left $fakeInstall.Executable -Right $fakeExecutable) -or
    $fakeInstall.ApplicationId -cne 'App' -or
    $fakeInstall.AppUserModelId -cne 'OpenAI.Codex_test!App') {
    throw 'Registered Appx package identity conversion failed.'
  }
  $fakeSkinRoot = Join-Path $temporaryRoot 'skin-root'
  $fakeRuntimeInstall = Sync-DreamSkinCodexRuntime -Codex $fakeInstall -SkinRoot $fakeSkinRoot
  $expectedRuntimeRoot = Join-Path $fakeSkinRoot 'runtime\1.2.3.4'
  $expectedRuntimeExecutable = Join-Path $expectedRuntimeRoot 'app\ChatGPT.exe'
  if ($null -eq $fakeRuntimeInstall -or
    -not (Test-DreamSkinPathEqual -Left $fakeRuntimeInstall.Executable -Right $expectedRuntimeExecutable) -or
    -not (Test-DreamSkinPathEqual -Left $fakeRuntimeInstall.OfficialExecutable -Right $fakeExecutable) -or
    -not (Test-Path -LiteralPath $expectedRuntimeExecutable) -or
    -not (Test-Path -LiteralPath (Join-Path $expectedRuntimeRoot 'runtime-source.json'))) {
    throw 'The verified Store package was not mirrored into a private runtime safely.'
  }
  if (-not (Test-DreamSkinCodexExecutablePath -Codex $fakeRuntimeInstall -Path $fakeExecutable) -or
    -not (Test-DreamSkinCodexExecutablePath -Codex $fakeRuntimeInstall -Path $expectedRuntimeExecutable) -or
    (Test-DreamSkinCodexExecutablePath -Codex $fakeRuntimeInstall -Path (Join-Path $temporaryRoot 'other.exe'))) {
    throw 'Runtime and official Codex executable identity matching is unsafe.'
  }
  $activationPlan = Get-DreamSkinCodexActivationPlan -Codex $fakeInstall -ArgumentList @(
    '--remote-debugging-address=127.0.0.1',
    '--user-data-dir=C:\Profile With Space'
  )
  if ($activationPlan.AppUserModelId -cne 'OpenAI.Codex_test!App' -or
    $activationPlan.Arguments -cne '--remote-debugging-address=127.0.0.1 "--user-data-dir=C:\Profile With Space"') {
    throw 'Packaged Codex activation plan was not formed safely.'
  }
  ...
  $runtimePackageState = [pscustomobject]@{
    schemaVersion = 4
    codexExe = $expectedRuntimeExecutable
    codexOfficialExe = $fakeExecutable
    codexRuntimeRoot = $expectedRuntimeRoot
    codexPackageRoot = $fakePackageRoot
    codexVersion = '1.2.3.4'
    codexPackageFullName = $fakePackage.PackageFullName
    codexPackageFamilyName = $fakePackage.PackageFamilyName
  }
  $resolvedRuntimeInstall = Resolve-DreamSkinCodexInstallFromState -State $runtimePackageState `
    -RegisteredInstalls @($fakeInstall) -SkinRoot $fakeSkinRoot
  if ($null -eq $resolvedRuntimeInstall -or
    -not (Test-DreamSkinPathEqual -Left $resolvedRuntimeInstall.Executable -Right $expectedRuntimeExecutable) -or
    -not (Test-DreamSkinPathEqual -Left $resolvedRuntimeInstall.OfficialExecutable -Right $fakeExecutable)) {
    throw 'A valid private-runtime state did not resolve against its registered Store package.'
  }
  $runtimePackageState.codexRuntimeRoot = Join-Path $temporaryRoot 'outside-runtime'
  if ($null -ne (Resolve-DreamSkinCodexInstallFromState -State $runtimePackageState `
    -RegisteredInstalls @($fakeInstall) -SkinRoot $fakeSkinRoot)) {
    throw 'A private-runtime state outside the skin root was accepted.'
  }
  ...
  if ($initialTheme.Theme.id -cne 'preset-midnight-aurora' -or
    $initialTheme.Theme.name -cne '午夜极光' -or
    ...
    throw 'Default Windows theme did not seed the Midnight Aurora wallpaper contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'preset-midnight-aurora' -or
    $preseededThemes[0].Name -cne '午夜极光') {
    throw 'Midnight Aurora was not preseeded in the Windows saved-theme menu.'
  }
  ...
  foreach ($requiredFallbackBehavior in @(
      "[ValidateSet('Auto', 'Official', 'Private')]",
      'The Store activation path failed:',
      'did not preserve Chromium debugging arguments',
      'Sync-DreamSkinCodexRuntime -Codex $currentCodex'
    )) {
    if (-not $startSource.Contains($requiredFallbackBehavior)) {
      throw "The Windows Store launch fallback is incomplete: $requiredFallbackBehavior"
    }
  }
  ...
  foreach ($powerShellFile in Get-ChildItem -LiteralPath $Root -Recurse -File -Filter '*.ps1') {
    ...
    if ($sourceText -match '[^\x00-\x7F]' -and
      ($sourceBytes.Length -lt 3 -or $sourceBytes[0] -ne 0xEF -or
        $sourceBytes[1] -ne 0xBB -or $sourceBytes[2] -ne 0xBF)) {
      throw "PowerShell 5.1 requires a UTF-8 BOM for non-ASCII source: $($powerShellFile.FullName)"
    }
  }

  $easyInstall = Join-Path $Root 'scripts\install-windows.ps1'
  $easyLaunch = Join-Path $Root 'scripts\launch-windows.ps1'
  foreach ($entry in @($easyInstall, $easyLaunch)) {
    if (-not (Test-Path -LiteralPath $entry)) { throw "The user-facing Windows entry is missing: $entry" }
  }
  $powershell = (Get-Command powershell.exe -ErrorAction Stop).Source
  $installPlan = (@(& $powershell -NoProfile -ExecutionPolicy Bypass -File $easyInstall -DryRun) -join "`n") |
    ConvertFrom-Json
  if (-not $installPlan.installScript -or -not $installPlan.launchScript -or -not $installPlan.errorLog) {
    throw 'The easy installer dry run did not report a complete plan.'
  }
  $launchPlan = (@(& $powershell -NoProfile -ExecutionPolicy Bypass -File $easyLaunch -DryRun) -join "`n") |
    ConvertFrom-Json
  if (-not $launchPlan.startScript -or -not $launchPlan.errorLog -or -not $launchPlan.promptRestart) {
    throw 'The easy launcher dry run did not report a complete plan.'
  }
  ...
  & $node.Path (Join-Path $Root 'scripts\injector.mjs') --check-payload --theme-dir $oversizedTheme `
    1>$null 2>$null
