...
- Public distribution now uses the programmatic Midnight Aurora abstract wallpaper as the bundled default.
- Removed portrait, character, branded, and unsanitized screenshot examples from the repository and documentation.
- The bundled saved-theme directory is derived from `theme.json.id` instead of a hard-coded preset name.

## 1.2.1 — 2026-07-17

### 改进

- 新增可双击的 Install / Launch / Restore 入口；失败时保留弹窗并把详情写入 `%LOCALAPPDATA%\CodexDreamSkin`，不再只留下闪退终端。
- 默认 `Auto` 启动先尝试 Microsoft Store 应用身份；若系统丢弃 Chromium 调试参数，自动复制当前已验证的官方安装到 gitignored 私有运行目录并重试，同时沿用 Store 用户配置。
- 私有运行副本按 Codex 版本缓存并在 Store 更新后刷新；状态 schema 4 同时记录官方路径与私有路径，恢复时重新校验 Store 包身份。
- 为包含中文的 PowerShell 文件补 UTF-8 BOM，确保 Windows PowerShell 5.1 不会把源码误读为本地 ANSI 编码。

### 修复

- 原子配置替换不再向 `File.Replace` 传入空备份路径，兼容 Windows PowerShell 5.1 / .NET Framework。
- 多行 TOML 数组检查兼容 CRLF；恢复带引号的 `[desktop]` 键时不再额外添加空行。
...
- Windows 首次安装会把 UI-free 的程序化抽象主题「午夜极光」设为活动主题并播种到「已保存主题」，无需再从 macOS 目录手动导入。
