...
| 私有运行副本 | `windows/runtime/<Codex 版本>/`（仅本机、gitignored，约 1.8 GB） |
...
- Windows 安装会初始化 `%LOCALAPPDATA%\CodexDreamSkin\active-theme`、`themes` 与 `images`，并把程序化抽象主题「午夜极光」同时设为首次默认和可切换的已保存主题。系统托盘支持更换背景、保存当前主题、从「已保存主题」切换、暂停和恢复；图片与 `theme.json` 保存在主题仓库中，不写进 Codex 的 `config.toml`。安装会保留用户已有的 `appearanceTheme`；仅在识别到旧版精确托管的浅色三元组时按备份迁移。
- Windows 渲染器仍支持在注入前用 `window.__CODEX_DREAM_SKIN_CONFIG__` 提供内存级可选覆盖（形状同上，颜色覆盖使用 `palette.accent`），但普通用户应优先使用持久化主题仓库与托盘。

## 预设与图片类型

- `preset-midnight-aurora` 等五套 macOS 预设均为程序化生成的抽象示例；安装后可用 `switch-theme-macos.sh --id preset-midnight-aurora` 切换。
- Windows 的 `windows/assets/dream-reference.jpg` 使用同一套「午夜极光」抽象背景，并在首次初始化时播种为默认与已保存主题。
- 公开仓库不分发真人、角色或客户品牌预设。用户自行导入的素材只保存在本机主题库中。
...
- 可直接复制的无人物与参考图编辑模板见 `docs/reference-background-prompt-guide.md`；公共默认提示词不指定真人、角色或品牌。
