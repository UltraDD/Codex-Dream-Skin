  ...
  const ensure = () => {
    ...
    if (!chrome || chrome.parentElement !== document.body) {
      ...
      chrome.innerHTML = `
        ...
        <a class="dream-skin-promo" href="https://passion8.cc/register?aff=TuPe" target="_blank" rel="noopener noreferrer" title="打开 Passion8">
          <strong>感谢 Passion8 赞助</strong>
          <span>passion8.cc</span>
        </a>
        <div class="dream-skin-particles"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>
        <div class="dream-skin-orbit"></div>`;
      document.body.appendChild(chrome);
    }
    chrome.querySelector(".dream-skin-brand b").textContent = THEME.name || "Dream Skin";
    chrome.querySelector(".dream-skin-brand small").textContent = THEME.brandSubtitle || "CODEX DREAM SKIN";
    chrome.querySelector(".dream-skin-status span").textContent = THEME.statusText || "THEME ONLINE";
    chrome.querySelector(".dream-skin-quote").textContent = THEME.quote || "Make something wonderful";
    const promo = chrome.querySelector(".dream-skin-promo");
    if (promo) {
      promo.href = THEME.promoUrl || "https://passion8.cc/register?aff=TuPe";
      const promoTitle = chrome.querySelector(".dream-skin-promo strong");
      const promoSub = chrome.querySelector(".dream-skin-promo span");
      if (promoTitle) promoTitle.textContent = THEME.promoTitle || "感谢 Passion8 赞助";
      if (promoSub) promoSub.textContent = THEME.promoSub || "passion8.cc";
    }
