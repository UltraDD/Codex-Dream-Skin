...
- Public presets are now limited to the five programmatically generated abstract themes.
- Removed portrait, character, and unsanitized runtime examples from public distribution.
- Standalone documentation packaging no longer expects removed gallery or portrait-preview assets.
...
- 内置 5 套可复现的程序化抽象预设；安装后幂等播种到主题库，菜单栏或 `switch-theme-macos.sh` 可直接切换，且绝不覆盖 `custom-*` 用户主题。
...
- 公开发行面只保留程序化抽象预设与脱敏示例；用户提供的图像不自动获得 MIT 素材许可，公开再分发前仍需确认相应权利。
...
- 示例纯横幅：`docs/images/hero-banner-red-white.png`（无人机 UI 合成）
